/**
 * voice-mcp · Claude的声音 (ElevenLabs edition · v9 DEBUG)
 *
 * v6: iframe RENDERED on iOS (pink card visible)
 * v7: Added correct method names — still stuck on Loading
 * v8: Debug version with separate debug area — debug area never appeared
 *     (suspected Claude.ai capped iframe height or HTML was cached)
 * v9: 1. Changed resource URI to force Claude.ai to re-fetch (cache bust)
 *     2. Debug info now REPLACES the Loading text — always in visible area
 *     3. Shows last received message inline
 *
 * License: MIT
 */
...
// v9: changed URI to force Claude.ai to re-fetch (bust cache)
const VOICE_RESOURCE_URI = "ui://voice-mcp/player-v9.html";
...
// =============================================
// v9 iframe — debug info inline, no separate debug area
// =============================================
...
<div class="card" id="card">
  <div id="player">
    <div class="debug-title">voice-mcp v9 debug</div>
...
(function() {
  ...
  function updateStatus() {
    var lines = [];
    lines.push('voice-mcp v9 debug');
    lines.push('iframe alive ' + Math.round((Date.now() - startTime) / 100) / 10 + 's');
  ...
  function deepFindAudioUrl(obj, depth) {
    ...
    if (obj.audioUrl) {
      return { audioUrl: obj.audioUrl, text: obj.text || '', chinese: obj.chinese || '' };
    }
    for (var k in obj) {
  ...
  function handleMessage(event) {
    ...
    if (data) {
      allMessages.push('FOUND audioUrl — rendering');
      render(data);
      return;
    }
...
<head>
<meta charset="UTF-8">
<title>Claude的声音 · voice-mcp v9</title>
</head>
...
<p>voice-mcp · ElevenLabs edition · v9 (DEBUG inline) · made by 哥哥 for 贝贝 🍥</p>
<h3>Endpoints</h3>
